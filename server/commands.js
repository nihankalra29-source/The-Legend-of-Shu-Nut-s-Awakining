...
function handleCommand(ctx, rawText) {
  ...
  if (cmd === '/help') {
    ...
    if (actorRank >= RANK.owner) {
      lines.push('/op <player> - grant admin', '/deop <player> - revoke admin');
    }
  ...
  if (cmd === '/tpaccept' || cmd === '/tpdeny') {
    ...
    requester.x = actor.x;
    requester.y = actor.y;
    world.sendTo(requester.id, { type: 'teleport', x: actor.x, y: actor.y });
  ...
  if (cmd === '/tp') {
    ...
    actor.x = target.x;
    actor.y = target.y;
    ctx.send({ type: 'teleport', x: target.x, y: target.y });
  ...
  if (cmd === '/op' || cmd === '/deop') {
    ...
    send(cmd === '/op' ? `${user.username} is now an admin.` : `${user.username} is no longer an admin.`);
    return;
  }
...
module.exports = { handleCommand, parseDuration, formatDuration, RANK };
